...
export default function RecruitmentDashboard({
  ...
  const incompleteScreenings = applicants.filter((a) => a.phase === 1 && !a.testScores);
  const readyForCV = applicants.filter((a) => a.phase === 3 && a.status.includes('CV'));
  const waitingEmployer = applicants.filter((a) => a.phase === 4);

  // Recent Management Activity
  const managementActivity = activityLogs
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200">
        <div className="px-6 py-4 border-b border-slate-200 bg-[#8B5CF6]/5 flex items-center justify-between">
          ...
          <span className="px-3 py-1 bg-[#8B5CF6] text-white text-xs font-bold rounded-full">
            {pendingMedicalValidations.length} Waiting
          </span>
        </div>
        <div className="divide-y divide-slate-100">
          {pendingMedicalValidations.length === 0 ? (
            ...
            pendingMedicalValidations.map((applicant) => (
              <div key={applicant.id} className="p-4 hover:bg-slate-50 transition-colors flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#64748B]">{applicant.applicantCode || applicant.id} • Returned from clinic</p>
                  </div>
                </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200">
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
          ...
          <span className="text-xs font-bold text-[#64748B]">{incompleteScreenings.length} Pending</span>
        </div>
        <div className="divide-y divide-slate-100">
          {incompleteScreenings.length === 0 ? (
            ...
            incompleteScreenings.map((applicant) => (
              <div key={applicant.id} className="p-4 hover:bg-slate-50 transition-colors flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-bold text-[#0F172A]">{applicant.name}</p>
                    <p className="text-xs text-[#64748B]">{applicant.applicantCode || applicant.id} • Needs exam scoring</p>
                  </div>
                </div>
                <button
                  onClick={() => onNavigate('screening')}
