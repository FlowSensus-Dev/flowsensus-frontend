...
export default function AdministrationDashboard({ applicants = [], onViewApplicant = () => {}, onNavigate = () => {} }) {
  // Action Queues
  const expired_docs = 2; // Mock data for demo
  const expiring_in_30 = applicants?.filter((a) => a.phase >= 3)?.length; // Mock
  const expiring_in_60 = applicants?.filter((a) => a.phase >= 2)?.length; // Mock
  const pending_ocr = applicants?.filter((a) => a.phase === 5 && a.status.includes('Final')).slice(0, 3);
  const newly_unlocked = applicants?.filter((a) => a.phase === 5);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200">
        <div className="px-6 py-4 border-b border-slate-200 bg-[#EF4444]/5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 bg-[#EF4444] text-white text-xs font-bold rounded-full">{expired_docs} Expired</span>
            <span className="px-3 py-1 bg-[#F59E0B] text-white text-xs font-bold rounded-full">{expiring_in_30} 30-Day</span>
            <span className="px-3 py-1 bg-yellow-500 text-white text-xs font-bold rounded-full">{expiring_in_60} 60-Day</span>
          </div>
        </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-[#0F172A] text-sm">
                    {alert.doc} Ã¢â‚¬Â¢ {alert.applicant}
                  </p>
                  <p
                  ...
                  >
                    {alert.type === 'expired' ? `Expired ${alert.days_ago} days ago` : `Expires in ${alert.days_left} days`}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => onViewApplicant(alert.applicant_id)}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#64748B]">{applicant.id} Ã¢â‚¬Â¢ Data mismatch detected</p>
                  </div>
                </div>
                <button
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200">
        <div className="px-6 py-4 border-b border-slate-200 bg-[#10B981]/5 flex items-center justify-between">
          ...
          <span className="text-xs font-bold text-[#64748B]">{newly_unlocked?.length} Ready</span>
        </div>
        <div className="divide-y divide-slate-100">
            ...
            newly_unlocked?.map((applicant) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-bold text-[#0F172A]">{applicant.name}</p>
                    <p className="text-xs text-[#64748B]">{applicant.id} Ã¢â‚¬Â¢ Ready for visa processing</p>
                  </div>
                </div>
                <button
                  onClick={() => onNavigate('ocr')}
