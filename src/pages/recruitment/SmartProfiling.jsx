...
export default function SmartProfiling({
  ...
  const calculateReadiness = (jobOrder) => {
    ...
    if (applicant.workExperience && applicant.workExperience.length > 0) {
      jobOrder.keyDuties.forEach((duty) => {
    ...
    if (applicant.skills && applicant.skills.length > 0) {
      const skillMatches = jobOrder.keyDuties.filter((duty) =>
    ...
    if (applicant.certifications && applicant.certifications.length > 0) {
      const certMatches = jobOrder.certifications.filter((cert) =>
    ...
    if (applicant.languagesSpoken && applicant.languagesSpoken.length > 0) {
      const langMatches = jobOrder.languageRequirements.filter((lang) =>
  ...
  const handleEndorse = (jobOrderName) => {
    if (!applicant) return;
