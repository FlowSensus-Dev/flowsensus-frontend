...
function analyzeEmployment(records) {
  const flags = [];
...
export default function Registration({
  ...
  const proofUpload = (onFile, existingName) => {
  ...
  const setId = (id, k, v) => setIds(p => p.map(x => x.id === id ? { ...x, [k]: v } : x));
  const removeId = (id) => setIds(p => p.filter(x => x.id !== id));
  ...
  const setEdu = (id, k, v) => setEducation(p => p.map(x => x.id === id ? { ...x, [k]: v } : x));
  const removeEdu = (id) => setEducation(p => p.filter(x => x.id !== id));
  ...
  const setCert = (id, k, v) => setCerts(p => p.map(x => x.id === id ? { ...x, [k]: v } : x));
  const removeCert = (id) => setCerts(p => p.filter(x => x.id !== id));
  ...
  const setTraining = (id, k, v) => setTrainings(p => p.map(x => x.id === id ? { ...x, [k]: v } : x));
  const removeTraining = (id) => setTrainings(p => p.filter(x => x.id !== id));
  ...
  const setLang = (id, k, v) => setLanguages(p => p.map(x => x.id === id ? { ...x, [k]: v } : x));
  const removeLang = (id) => setLanguages(p => p.filter(x => x.id !== id));
  ...
  const removeEmp = (id) => { setEmployment(p => p.filter(x => x.id !== id)); setFlagsAnalyzed(false); };
  ...
  const handleSave = () => {
    ...
    if (updateApplicant) {
      updateApplicant(selectedApplicantId, {
        ...
        age: parseInt(personal.age), sex: personal.sex,
        religion: personal.religion, civilStatus: personal.civilStatus,
